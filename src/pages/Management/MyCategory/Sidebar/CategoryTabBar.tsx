...
function CategoryTabBar({ currentType }: CategoryTabBarProps) {
  ...
  return (
    <Container>
      <TabBar
        $active={CATEGORY_TYPE[currentType].key === '퀴즈'}
        onClick={() => navigate(`/management/mycategory?type=quiz`)}
      >
        <Typography variant="button" color={CATEGORY_TYPE[currentType].key === '퀴즈' ? 'mainMintDark' : 'grayScale03'}>
          퀴즈
        </Typography>
      </TabBar>
      <TabBar
        $active={CATEGORY_TYPE[currentType].key === '요약'}
        onClick={() => navigate(`/management/mycategory?type=summary`)}
      >
        <Typography variant="button" color={CATEGORY_TYPE[currentType].key === '요약' ? 'mainMintDark' : 'grayScale03'}>
          요약
        </Typography>
      </TabBar>
    </Container>
